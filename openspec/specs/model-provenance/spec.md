...
followed by a digit-led version token — by matching its base identifier (the part before the
`@`) against the provenance map, while keeping the full literal as evidence. A trailing `@`
segment that does not begin with a digit SHALL NOT be treated as a version suffix, and the
literal SHALL remain unmatched.
...
- **THEN** no model reference is detected, because the `@` segment is not digit-led
