...
A `model_reference` detection SHALL bind to a provider detection in the same file, assigning the
resolved provenance to that flow. A `model_reference` with no provider detection in its file
SHALL stand alone as its own finding. Binding SHALL NOT cross file boundaries.
