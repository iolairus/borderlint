...
- [ ] 1.3 `--format sbom` does not gate CI — exit 0 even when a supplied policy would fail (carve-out in the CI exit-code path)

## 2. Tests

- [ ] 2.1 Tests: the envelope carries schema id + version + KB date; components list every flow with sorted jurisdiction(s) + sites; no `severity`/`level`/verdict field appears anywhere; a failing policy still emits all flows and exits 0; `--format sbom` without a policy exits 0; the same scan twice is byte-identical
